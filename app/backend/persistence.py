...
def _state_from_dict(data: dict[str, Any]) -> RuntimeState:
    ...
    return RuntimeState(
        ...
        theme=t,
        ui_lang=str(data.get("ui_lang", d0.ui_lang)).upper() if str(data.get("ui_lang", d0.ui_lang)).upper() in ("RU", "EN") else d0.ui_lang,
    ...
    with _profile_lock:
        ...
        payload = {
            ...
            "aim": {
                ...
                "scan_region": aim.get("scan_region", {"top": 100, "left": 100, "width": 300, "height": 300}),
            ...
            "gamepad": {
                "background_method": api.state.gamepad_background_method if hasattr(api.state, 'gamepad_background_method') else "sendinput",
            },
        }
    ...
    with _profile_lock:
        ...
        try:
            ...
            if a and hasattr(api, 'aim'):
                api.aim.update_config(float(a.get("confidence", 0.6)), int(a.get("smooth_steps", 10)), float(a.get("reset_delay", 0.2)))
                sr = a.get("scan_region") or {}
                if sr:
                    api.aim.set_scan_region(int(sr.get("top", 100)), int(sr.get("left", 100)), int(sr.get("width", 300)), int(sr.get("height", 300)))
            ...
            if m and hasattr(api, 'macro'):
                ...
                for act in m.get("actions") or []:
                    api.macro.add_action(act.get("key", "space"), float(act.get("delay", 0.5)), float(act.get("hold", 0.05)))
